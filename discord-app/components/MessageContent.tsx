...
export default function MessageContent({
  ...
  useEffect(() => {
    console.log("MessageContent useEffect triggered", { type, fileUrl, hasFetchFunc: !!fetchAuthenticatedMedia });

    if (type !== "m.image" || !fileUrl || !fetchAuthenticatedMedia) {
      console.log("MessageContent: Skipping image fetch", { type, fileUrl, hasFetchFunc: !!fetchAuthenticatedMedia });
      return;
    }
    ...
    const loadImage = async () => {
      console.log("MessageContent: Starting image load for:", fileUrl);
      try {
  ...
  if (type === "m.image" && fileUrl) {
    console.log("MessageContent: Image message detected", { type, fileUrl, loading, error, blobUrl });
    ...
    if (error || !blobUrl) {
      return (
        <div className="my-2 px-4 py-3 bg-red-900/20 border border-red-700/50 rounded-lg">
          <p className="text-sm text-red-300">Failed to load image: {content}</p>
        </div>
      );
    }
